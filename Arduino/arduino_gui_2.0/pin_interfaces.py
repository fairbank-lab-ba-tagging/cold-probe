...
class MyPin():
    'Pin class to hold pin object, name, num, and funcs'

    def __init__(self, label, pin_num, Pin):
        self.label = label
...
class Monitor(Interface):
    ...
    def __init__(self, key, board, frame, main_frame, label, analog_pin_num):
        ...
        # Data variables
        self.update_count = 0            # Keep track of times updated
        self.plot_mod = 10               # Plot every 10 (can change)
    ...
    def build_monitor(self):
        ...
        # Plot config options
        # ymax_label = tk.Label(master=plot_config_frame, text='Y max:')
        # ymin_label = tk.Label(master=plot_config_frame, text='Y min:')
        # ymax_entry = tk.Entry(master=plot_config_frame, width=8, textvariable=self.ymax,)
        # ymin_entry = tk.Entry(master=plot_config_frame, width=8, textvariable=self.ymin)
        num_x_label = tk.Label(master=plot_config_frame, text='Number of points:')
        mod_x_label = tk.Label(master=plot_config_frame, text='Plot Every:')
        num_x_entry = tk.Entry(master=plot_config_frame, width=8, textvariable=self.num_x)
        mod_x_entry = tk.Entry(master=plot_config_frame, width=8, textvariable=self.mod_x)
        ...
        # ymax_label.grid(column=0, row=0, sticky='e')
        # ymax_entry.grid(column=1, row=0)
        # ymin_label.grid(column=0, row=1, sticky='e')
        # ymin_entry.grid(column=1, row=1)
        num_x_label.grid(column=0, row=0, sticky='e')
        num_x_entry.grid(column=1, row=0)
        mod_x_label.grid(column=0, row=1, sticky='e')
        mod_x_entry.grid(column=1, row=1)
    ...
    def update(self):
        ...
        if self.update_count % self.plot_mod == 0:
            self.data_history.append(new_value)
            self.time_history.append(datetime.now())
            self.update_count = 0
        self.update_count += 1
